...
def safelog(log, usernam):
    ...
    logfile = open(u'/home/javier/apps/python/POMCAS/asocartest/static/log/%s_cartog_v1.log'%(usuario.username), 'w')
...
def excel_process(excel_addr, username):
    ...
    excel_file = xlrd.open_workbook('/home/javier/apps/python/POMCAS/asocartest/static/Matrix_loaded/%s'%(excel_addr))
        ...
        while excel_sheet.cell_value(colx =1, rowx =rowini) != '': 
            ...
            if valid:
                if elaboracion < fuente:
                    log += u'El año de elaboración (%s) no puede ser mayor a la fuente(%s) \n'%(fuente, elaboracion)
                    doprocess = False
            else: 
